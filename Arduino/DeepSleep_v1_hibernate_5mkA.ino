...
long sleepTime = 0;
const int wakeup_sec = 360; // wake up after 360 seconds
...
// WARNING!
// In the Arduino Core ESP32, calling esp_sleep_pd_config() can lead to an assert failed
// (refs >= 0) and a crash if the corresponding power domains were not activated via esp_sleep_enable_*.
// In Arduino Core ESP32 version 3.2.0, this bug is still present.
// Solution: avoid using esp_sleep_pd_config() in Arduino Core or use it only in PlatformIO/ESP-IDF.
// For simple deep sleep functionality, it is sufficient to use esp_sleep_enable_timer_wakeup() and esp_deep_sleep_start().
